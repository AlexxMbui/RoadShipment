    ...
    fun signup(name:String, email:String, password:String){
        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener {
            ...
            usersRef.setValue(userProfile).addOnCompleteListener {
                progress.dismiss()
                if (it.isSuccessful){
                    Toast.makeText(this.context, "Success,Let's Go", Toast.LENGTH_SHORT).show()
                    navController.navigate(LOGIN_URL)
                }else{
                    Toast.makeText(this.context, "Wrong Details Provided,Try Again", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
